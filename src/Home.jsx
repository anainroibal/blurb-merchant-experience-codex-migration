...
import { CHANGED, Mark } from "./SiteNav.jsx";
...
/* The four families the live page shows, in its order. Wall art is not
   on the home page today and is not added here — this screen is not the
   place to argue for a fifth card. The descriptions are Blurb's. */
const PRODUCTS = [
  ["photo",    "From high-end photography albums to keepsake family books, our most premium format, with multiple trim sizes and paper types."],
  ["trade",    "Ideal for books that combine art with text, or just text alone — portfolios, cookbooks, novels, children's books and the like."],
  ["magazine", "Great for a series or a one-off. Newsstand quality, and the easiest format to distribute."],
  ["notebook", "Blank, lined, square or dot-grid pages, plus the option to add photos or illustrations within them."],
];

/* The live "Tools and resources" tabs, with their real destinations. The
   Selling tab is where a checkout link belongs: it is the only tab whose
   audience has already decided to sell, and today it offers them the
   three retail channels and no way to see what they would keep. */
const TABS = [
  {
    id: "create", label: "Book creation", mark: null,
  ...
  {
    id: "print", label: "Printing", mark: null,
  ...
  {
    id: "sell", label: "Selling", mark: CHANGED,
    body: "Sell one book from a link you share, or list it where readers are already browsing. Five routes, and they work differently — the comparison is the point.",
    actions: [["Compare the ways to sell", "waystosell"], ["Work out what you keep", "margin"]],
  },
  {
    id: "business", label: "Business services", mark: null,
...
export default function Home({ onGo }) {
  ...
  return (
    <div style={{ fontFamily: FONT_BODY, color: T.textNeutral }}>
      ...
      <section style={{ background: T.bgAccentSubtle, borderBottom: `1px solid ${C.blue100}` }}>
        <div style={{ maxWidth: 900, margin: "0 auto", padding: "clamp(48px, 7vw, 88px) 24px", textAlign: "center" }}>
          ...
          {/* ── Two prompts, and they are the site's two real jobs ──
              ...
              Create opens /getting-started, which is where the options and
              the tools that make them live. Print opens /pdf-to-book. */}
          <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap" }}>
            <Btn onClick={() => onGo("getstarted")}>Create</Btn>
            <Btn tone="ghost" onClick={() => onGo("pdftobook")}>Print</Btn>
          </div>
          <p style={{ fontSize: TYPE.sm, color: T.textSubtle, margin: "16px 0 0" }}>
            Start from an idea and we will help you make it, or bring a finished PDF and we will print it.
          </p>
      ...
      <section style={{ maxWidth: 1080, margin: "0 auto", padding: "clamp(40px, 6vw, 64px) 24px 8px" }}>
        ...
        <div style={{ display: "grid", gap: 20, gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))" }}>
          ...
          <div style={{ border: `1px solid ${T.border}`, borderRadius: R.lg, padding: 24, display: "grid", gap: 12, alignContent: "start" }}>
            ...
            <div style={{ marginTop: 4 }}>
              <Btn size="md" onClick={() => onGo("waystosell")}>See my selling options</Btn>
            </div>
          </div>
        </div>
      </section>
      ...
      {/* ── Products ──
          Same four cards as the live page, same order, in its words. The
          only difference is where the price comes from. */}
      <section style={{ maxWidth: 1180, margin: "0 auto", padding: "clamp(40px, 6vw, 64px) 24px 0" }}>
        ...
        <div style={{ display: "grid", gap: 20, gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))" }}>
          {PRODUCTS.map(([id, blurb]) => {
            const f = CATALOG[id];
            const from = fromPrice(id);
            return (
              <button
                key={id}
                onClick={() => onGo(id === "photo" ? "product" : "getstarted", { format: id })}
                style={{
                  textAlign: "left", font: "inherit", cursor: "pointer",
                  border: `1px solid ${T.border}`, borderRadius: R.lg, background: T.bgNeutral,
                  padding: 0, display: "grid", gridTemplateRows: "auto 1fr", overflow: "hidden", minWidth: 0,
                }}
              >
                {/* Fixed header height so the image bands line up across the
                    row — "Notebooks & Journals" wraps to two lines and the
                    others do not. */}
                <div style={{ padding: "20px 20px 12px", minHeight: 96 }}>
                  <div style={{ fontFamily: FONT_DISPLAY, fontSize: TYPE["4xl"], fontWeight: 500, lineHeight: 1.2 }}>
                    {f.label}
                  </div>
                  <div style={{ fontSize: TYPE.sm, fontWeight: 700, color: T.textNeutral, marginTop: 4 }}>
                    {from == null ? "Price on request" : `From ${money(from)}`}
                  </div>
                </div>
                {/* Placeholder for the product photograph the live card carries. */}
                <div style={{ background: C.gray50, height: 120, borderTop: `1px solid ${C.gray100}` }} />
                <p style={{ margin: 0, padding: "14px 20px 20px", fontSize: TYPE.sm, lineHeight: 1.6, color: T.textSubtle }}>
                  {blurb}
                </p>
              </button>
            );
      ...
      <section style={{ maxWidth: 1080, margin: "0 auto", padding: "clamp(48px, 7vw, 72px) 24px 0" }}>
        ...
        >
          <div style={{ fontSize: TYPE.lg, fontWeight: 600 }}>
            {active.label}
